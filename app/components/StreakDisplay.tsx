...
import { updateUserStreak, getUserStreak, checkStreakMilestones } from '@/lib/users';
...
const StreakDisplay: React.FC<StreakDisplayProps> = ({ telegramId, onMilestoneReached }) => {
  ...
  useEffect(() => {
    const initializeStreak = async () => {
      try {
        ...
        const streakData = await updateUserStreak(telegramId);
        
        if (streakData) {
          setCurrentStreak(streakData.currentStreak);
          setHighestStreak(streakData.highestStreak);
          setStartDate(streakData.startDate);
          
          // Check if user has reached any milestones
          const reward = await checkStreakMilestones(telegramId);
          if (reward) {
  ...
  }, [telegramId, onMilestoneReached]);

  // Format date (e.g., "Mar 15, 2025")
